...
#define ZLIB_CONST
#define Z_PREFIX /*This should probably not be used*/
...
/* returns a string with version of zstdwrap library */
const char * zstdVersion(void);


/*** COMPRESSION ***/
/* ZWRAP_useZSTDcompression() enables/disables zstdwrap compression during runtime.
   By default zstdwrap compression is disabled. To enable zstdwrap compression please use one of the methods:
   ...
   All above-mentioned methods will enable zstdwrap compression for all threads.
...
/* checks if zstdwrap compression is turned on */
...
/* ZWRAP_setDecompressionType() enables/disables automatic recognition of zstdwrap/zlib compressed data during runtime.
   By default auto-detection of zstdwrap and zlib streams in enabled (ZWRAP_AUTO).
...
/* checks zstdwrap decompression type */
ZWRAP_decompress_type ZWRAP_getDecompressionType(void);

/* Checks if zstdwrap decompression is used for a given stream.
   If will return 1 only when inflate() was called and zstdwrap header was detected. */
